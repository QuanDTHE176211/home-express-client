...
import { transportNavItems } from "@/lib/transport-nav-config"
import { CheckCheck, Trash2, Search, Filter, Bell, Package, FileText, DollarSign, Star, Truck, AlertCircle } from "lucide-react"
export default function NotificationsPage() {
  ...
  }, [activeTab, typeFilter, currentPage, fetchNotifications, user])
  ...
  const getNavItems = () => {
    if (user?.role === "MANAGER") {
      return adminNavItems.map(item => ({
        ...item,
        icon: typeof item.icon === 'string' ? item.icon : item.icon.displayName || item.icon.name || "LayoutDashboard"
      }))
    } else if (user?.role === "TRANSPORT") {
      return transportNavItems.map(item => ({
        ...item,
        icon: typeof item.icon === 'string' ? item.icon : item.icon.displayName || item.icon.name || "LayoutDashboard"
      }))
    } else {
  ...
  const notificationTypes = [
    ...
    { value: "BOOKING_CREATED", label: "Đơn hàng mới", icon: Package },
    { value: "QUOTATION_RECEIVED", label: "Báo giá", icon: FileText },
    { value: "PAYMENT_RECEIVED", label: "Thanh toán", icon: DollarSign },
    { value: "REVIEW_RECEIVED", label: "Đánh giá", icon: Star },
    { value: "JOB_STARTED", label: "Công việc", icon: Truck },
    { value: "TRANSPORT_VERIFIED", label: "Hệ thống", icon: CheckCheck },
    { value: "TRANSPORT_REJECTED", label: "Hệ thống", icon: AlertCircle },
  ]

  return (
    <DashboardLayout navItems={getNavItems()} title="Thông báo">
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle>{activeTab === "all" ? "Tất cả thông báo" : "Thông báo chưa đọc"}</CardTitle>
                    <CardDescription>{filteredNotifications.length} thông báo</CardDescription>
                  </div>
        ...
        <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Bạn có chắc chắn muốn xóa {selectedIds.length} thông báo đã chọn? Hành động này không thể hoàn tác.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
